			...
			<div className={`flex gap-4`}>
				<div className="relative">
					<Image
						...
						height={0}
						width={0}
						className={`${stockOut ? "opacity-50" : ""} w-[72px] h-[72px] rounded-lg  md:w-[84px] md:h-[84px] `}
					/>
					{stockOut ? (
						<div className="w-full h-full rounded absolute left-0 top-0 flex items-center justify-center">
							<span className="text-red-500 text-[14px] rounded-lg whitespace-nowrap capitalize border-solid border bg-white border-red-500 px-1 py-[2px]">
				...
				<div className="flex flex-col justify-between w-full">
					<span
						...
						className={`text-primary font-bold text-base ${stockOut ? "opacity-50" : ""}`}
					>
					...
					<div className="flex justify-between items-center">
						...
						>
							...
							{old_price > new_price ? (
								<>
									...
									<div className="rounded-md px-2 py-1 whitespace-nowrap text-sm text-white bg-red-500">
						...
						{!stockOut ? (
							...
							>
								...
								<span className="ml-2 hidden md:inline">{translations["add-to-cart"] || "কার্টে রাখুন"}</span>
							</button>
						) : (
							<Link
								href={"/products"}
							...
							>
								<span className="hidden md:inline">{translations["find-similar"] || "সিমিলার খুঁজুন"}</span>
								<span className="inline md:hidden">{translations["find"] || "খুঁজুন"}</span>
							</Link>
						)}
					</div>
				</div>
